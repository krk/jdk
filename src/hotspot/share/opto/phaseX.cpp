...
void PhaseIterGVN::verify_PhaseIterGVN(bool deep, bool deep_revisit_converged) {
#ifdef ASSERT
  ...
  verify_optimize(deep, deep_revisit_converged);
#endif
}
#endif /* PRODUCT */
...
bool PhaseIterGVN::deep_revisit(uint& loop_count) {
  ...
  for (; round < max_deep_revisit_rounds; round++) {
    ...
    loop_count = 0;
    if (drain_worklist(loop_count)) return false;
...
void PhaseIterGVN::optimize(bool deep) {
  ...
  if (deep && UseDeepIGVNRevisit) {
     deep_revisit_converged = deep_revisit(loop_count);
    if (C->failing()) return;
  }

  NOT_PRODUCT(verify_PhaseIterGVN(deep && UseDeepIGVNRevisit, deep_revisit_converged);)
  C->print_method(PHASE_AFTER_ITER_GVN, 3);
}

#ifdef ASSERT
void PhaseIterGVN::verify_optimize(bool deep, bool deep_revisit_converged) {
  ...
  if (is_verify_Value() ||
    ...
    for (uint j = 0; j < worklist.size(); ++j) {
      ...
      if (is_verify_Value()) {
        verify_Value_for(n, false /* strict */);
      }
      if (is_verify_Ideal()) {
        verify_Ideal_for(n, false /* can_reshape */, deep);
        verify_Ideal_for(n, true  /* can_reshape */, deep);
      }
...
void PhaseIterGVN::verify_Value_for(const Node* n, bool strict) {
  ...
  // Skip if deep revisit is not attempted or did not converge.
